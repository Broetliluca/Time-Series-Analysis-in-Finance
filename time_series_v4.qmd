...
format:
  pdf:
    documentclass: scrartcl   # or article
    toc: true
...
```{r, plot_returns, cache=TRUE, echo=FALSE, fig.height=3.5}
...
merged %>%
  ...
  geom_line() +
  labs(
    title = "Price Levels: EUA Carbon Price, Paris-Aligned ETF, Oil & Gas ETF",
    y = "Price / NAV",
    x = "Date"
  ) +
  theme_minimal()
...
```{r, rolling, cache=TRUE, echo=FALSE, fig.height=3.5}
...
```{r corrmatrix, cache=TRUE, echo=FALSE, fig.height=3.5}
...
**Do changes in carbon prices cause changes in ETF returns?**<br>
...
garchx_coef_table <- as_tibble(coefs, rownames = "Parameter") %>%
  mutate(
    Parameter = recode(
      ...
      mu     = "Mean (μ)",
      omega  = "Constant (ω)",
      alpha1 = "ARCH effect (alpha)",
      beta1  = "GARCH effect (beta)",
...
We estimate a GARCH-X model for the Paris-Aligned ETF to test whether EUA carbon price returns directly affect volatility. The results confirm significant ARCH and GARCH effects, indicating volatility clustering and moderate persistence (alpha + beta ≈ 0.89).
...
# Appendix - Codelines by Chapter and task
